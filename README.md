...
新增了通用 ajax 请求类 `request`，该类对 axios 进行了一定的封装，尽可能地保证优化使用的同时不对开发者进行过多的限制。
...
const { server, request: $https } = bot.request.register( {
...
```ts
import { FetchServer } from "@/modules/request";
