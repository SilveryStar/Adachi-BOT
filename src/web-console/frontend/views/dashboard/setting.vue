		...
		<el-form v-if="setting" :model="setting" class="config-form" @submit.prevent>
			<div class="config-section">
				...
				<form-item label="登录平台">
					<el-radio-group v-model="setting.platform" :disabled="pageLoading"
					                @change="updateConfig('platform')">
				...
				<form-item label="邀请入群权限" desc="邀请 BOT 入群时，BOT 自动接受入群邀请的权限等级。">
					<el-radio-group v-model="setting.inviteAuth" :disabled="pageLoading"
					                @change="updateConfig('inviteAuth')">
				...
				<form-item label="阈值使用限制" desc="开启后当用户使用超过阈值时，本小时内 BOT 将不再响应其指令。">
					<el-switch v-model="setting.ThresholdInterval" :disabled="pageLoading"
					           @change="updateConfig('ThresholdInterval')"/>
				</form-item>
			</div>
			<div class="config-section">
				...
				<form-item label="模糊匹配"
					...
					<el-switch v-model="setting.fuzzyMatch" :disabled="pageLoading"
					           @change="updateConfig('fuzzyMatch')"/>
				</form-item>
				<form-item label="参数校验" desc="开启后若指令参数错误，BOT 将会给予提示。">
					<el-switch v-model="setting.matchPrompt" :disabled="pageLoading"
					           @change="updateConfig('matchPrompt')"/>
				</form-item>
				<spread-form-item
				...
				<form-item label="帮助信息样式" desc="指令help响应信息所展示的样式。">
					<el-radio-group v-model="setting.helpMessageStyle" :disabled="pageLoading"
					                @change="updateConfig('helpMessageStyle')">
			...
			<div class="config-section">
				...
				<form-item label="开启安全连接" desc="一般情况下开启时port应为为465，反之则为587或25。">
					<el-switch v-model="setting.mailConfig.secure" :disabled="pageLoading"
					           @change="updateConfig('mailConfig.secure')"/>
				</form-item>
				<template v-if="setting.mailConfig.secure">
					...
					<form-item label="证书校验" desc="建议关闭，开启可能会存在认证问题">
						<el-switch v-model="setting.mailConfig.rejectUnauthorized" :disabled="pageLoading"
						           @change="updateConfig('mailConfig.rejectUnauthorized')"/>
					</form-item>
				</template>
				<form-item label="离线发送邮件" desc="当 BOT 意外掉线时，向 Master QQ邮箱发送邮件提醒">
					<el-switch v-model="setting.mailConfig.logoutSend" :disabled="pageLoading"
					           @change="updateConfig('mailConfig.logoutSend')"/>
				</form-item>
...
const setting = ref<BotConfig | null>( null );
...
async function updateConfig( field ) {
	...
	const value = objectGet( setting.value, field );
	const data = {};
	objectSet( data, field, value );
	try {
		await $http.CONFIG_SET.post( { fileName: "setting", data } );
		ElNotification( {
			title: "成功",
			message: "更新成功。",
			type: "success",
