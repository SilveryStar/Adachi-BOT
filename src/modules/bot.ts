...
export default class Adachi {
	...
	constructor( root: string ) {
		...
		const logger = <log.Logger>client.logger;
		...
		const redis = new Database(config.dbPort, logger, config.dbPassword);
...
====================================================================`
	...
	private qrcodeLogin() {
		...
		const d = new Date();
		const job: Job = scheduleJob(d.setMinutes(d.getMinutes() + 2), () => {
			this.bot.client.login().then();
		} );
	...
	private login(): void {
		...
		this.bot.client.on( "system.login.slider", () => {
			...
			const job: Job = scheduleJob( "0/5 * * * * *", () => {
				...
				if ( file && file.trim() ) {
					this.bot.client.submitSlider(file.trim()).then();
					job.cancel();
			...
			process.stdin.once( "data", ( input ) => {
				this.bot.client.submitSlider(input.toString()).then();
				job.cancel();
			} );
		} );
		/* 处理设备锁事件 */
		this.bot.client.on( "system.login.device", ( { phone } ) => {
			if ( phone ) {
				...
				this.bot.client.sendSmsCode().then();
				const d = new Date();
				// 创建空的code.txt
				const dirName = `src/data/${ number }`;
				...
				const job: Job = scheduleJob( "0/5 * * * * *", () => {
					...
					if ( file && file.trim() ) {
						this.bot.client.submitSmsCode(file.trim()).then();
						job.cancel();
				...
				/* 兼容终端输入 */
				process.stdin.once( "data", ( input ) => {
					this.bot.client.submitSmsCode(input.toString()).then();
					job.cancel();
				} );
			} else {
				this.qrcodeLogin();
		...
		if ( this.bot.config.qrcode ) {
			...
			/* 账密登录 */
			this.bot.client.login(this.bot.config.number, this.bot.config.password).then();
		}
	}
